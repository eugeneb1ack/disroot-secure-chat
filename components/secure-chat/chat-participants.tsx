...
export function ChatParticipants({ view, connected }: { view: ChatView; connected: boolean }) {
  ...
  return <div className="chat-presence">
    <button type="button" ref={trigger} className="chat-presence-trigger" aria-expanded={open} aria-controls="chat-participants-panel" aria-haspopup="dialog" onClick={() => setOpen(value => !value)}><span className="chat-presence-avatars" aria-hidden="true">{members.slice(0, 4).map(member => <span key={member.id} className={`chat-presence-avatar is-${member.status}`}>{member.nickname.slice(0, 1).toUpperCase()}</span>)}</span><span className="chat-presence-summary"><span className="chat-presence-names">{members.slice(0, 2).map(member => member.nickname).join(", ")}{members.length > 2 ? ` +${members.length - 2}` : ""}</span><span className="chat-presence-count" aria-live="polite">{connected ? <><strong>{online}</strong> {t("online", "онлайн")}{!!view.presence?.joining && <small> · {t("joining", "подключаются")}: {view.presence.joining}</small>}</> : t("Reconnecting…", "Восстанавливаем связь…")}</span></span><span className="chat-presence-toggle">{t("Participants", "Участники")} <span aria-hidden="true">⌄</span></span></button>
    {open && <ChatPopover id="chat-participants-panel" label={t("Participants", "Участники")} anchor={trigger} onClose={() => setOpen(false)}><div className="chat-presence-list">
