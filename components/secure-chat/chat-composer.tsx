...
export function ChatComposer({ draft, setDraft, reply, onCancelReply, onSend, onEnd, identity, ready, connected, busy, error }: {
  ...
  return <form className="chat-composer" onSubmit={onSend}>
    ...
    <div className="chat-composer-foot"><span className="chat-current-alias" title={prettyFingerprint(identity.id)}><span aria-hidden="true">⌑</span> {identity.nickname}</span><span className="chat-compose-hint" role={busy ? "status" : undefined}>{busy ? t("Sending…", "Отправляем…") : draft.length >= MAX_TEXT * .9 ? `${draft.length} / ${MAX_TEXT}` : t("Shift + Enter for a new line", "Shift + Enter — новая строка")}</span><button type="button" className="chat-text-button" title={t("End this session and start a new conversation", "Завершить эту сессию и создать новую беседу")} onClick={onEnd}>{t("New chat", "Новый чат")}</button></div>
  </form>;
}
