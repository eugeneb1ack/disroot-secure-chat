...
test("room capacity reclaims closed conversations without evicting offline sessions", async () => {
  const store = new SecureChatStore();
  const invites = [], members = [];
  for (let i = 0; i < 16; i++) {
    const invite = { room: randomId(), secret: hex(crypto.getRandomValues(new Uint8Array(32))), founder: "a".repeat(64), expires: Date.now() + CHAT_TTL - 1000 };
    invites.push(invite); members.push(await storeLogin(store, invite, true));
  }
  // A disconnected browser is recoverable: never evict merely for being offline.
  for (const session of store.sessions.values()) session.lastSeen = 0;
  const fresh = { ...invites[0], room: randomId() };
  await assert.rejects(() => storeLogin(store, fresh, true), /capacity/);
  const lease = store.claim(members[0].token, 0);
  store.publish(members[0].token, { lease: lease.id, id: randomId(), wire: "A".repeat(64) });
  store.logout(members[0].token);
  const replacement = await storeLogin(store, fresh, true);
  assert.equal(store.rooms.size, 16);
  assert.equal(store.rooms.has(invites[0].room), false, "unreachable ciphertext is reclaimed");
  assert.equal(store.keyBindings.size, 17, "revoked keys remain bound until expiry");
  for (const member of members.slice(1)) assert.equal(store.snapshot(member.token, 0).ready, true);
  assert.equal(store.snapshot(replacement.token, 0).ready, true);
});
